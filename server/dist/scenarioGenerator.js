...
const youthAgeThresholds = {
    strict: 28,
    balanced: 32,
    relaxed: 38,
};
const seniorAgeThresholds = {
    strict: 65,
    balanced: 60,
    relaxed: 55,
};
...
const buildNightCondition = (variant) => {
...
const buildYouthCustomerCondition = (variant) => {
...
const buildSeniorCustomerCondition = (variant) => {
...
const buildChannelCondition = (channelLabel, channelValue) => {
    return () => ({
        label: '거래 채널',
...
const buildLunchCondition = (variant) => {
...
function buildCondition(keyword, variant) {
...
function generateTempId(order) {
    return `TEMP-${order}-${Math.random().toString(36).slice(2, 8)}`;
}
function generateScenarios(keywordsInput) {
